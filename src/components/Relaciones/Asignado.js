...
function Asignado() {
  let idPersonal = window.location.href.split('/')[window.location.href.split(
      '/').length - 1];
  ...
  const eliminar = async (dat) => {
    ...
    let result = await funciones.eliminarFila(
        'asignado/' + idPersonal + '/' + dat.codecentrosalud + '/' +
        dat.dateasignado);
    return result;
  };
  ...
  const modificar = async (dat) => {
    let result = await funciones.modificarFila(
        'asignado/' + idPersonal + '/' + modifi.codecentrosalud + '/' +
        dat.dateasignado,
        modificable);
    return result;
  };
              ...
              <Card sx={{width: '100% '}}>
                ...
                <CardContent>
                  <Typography gutterBottom variant="h3"
                              component="div">
                    Centros de salud asignados de la persona {idPersonal}
                  </Typography>
                ...
                <table>
                  ...
                  {datos.map((dato) => {
                    ...
                    return (
                        <tr>
                          <th>
                            {modifi != null && modifi.codecentrosalud ===
                            dato.codecentrosalud && modifi.dateasignado ===
                            dato.dateasignado ?
                                <input onChange={handleChangeModifi}
                                       type="text" id="codecentrosalud"
                                       spellCheck="false"
                                       placeholder={dato['codecentrosalud']}
                                       name="codecentrosalud"/> :
                                dato['codecentrosalud']}
                          </th>
                          <th>{dato['name']}</th>
                          <th>{funciones.formatDate(dato['dateasignado'])}</th>
