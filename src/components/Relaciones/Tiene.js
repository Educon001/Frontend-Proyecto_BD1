...
function Tiene() {
  let idVariante=window.location.href.split('/')[window.location.href.split('/').length-1]
  const [datos, setDatos] = useState([]);
  const showData = async () => {
    await funciones.getDatos('tiene/'+idVariante,setDatos)
  ...
  const eliminar = async (dat) => {
    ...
    let result = await funciones.eliminarFila('tiene/'+idVariante+'/'+ dat.codesintoma);
    return result;
  };
  ...
  const modificar = async (dat) => {
    let result = await funciones.modificarFila('tiene/' +idVariante+'/'+dat.codemedicamento,
        modificable);
    return result;
  };
              ...
              <Card sx={{width: '100% '}}>
                ...
                <CardContent>
                  <Typography gutterBottom variant="h3"
                              component="div">
                    Síntomas de la Variante {idVariante}
                  </Typography>
